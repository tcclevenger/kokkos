...
template <typename DeviceType, typename Layout,typename Contribution, typename Op, typename NumberType>
...
template <typename DeviceType, typename Layout, typename Op, typename NumberType>
...
struct TestDuplicatedScatterView {
  TestDuplicatedScatterView(int n) {
    ...
    test_duplicated_scatter_view<DeviceType, Kokkos::LayoutRight, ScatterType, NumberType>
        test_sv_right_config;
    test_sv_right_config.run_test(n);
    test_duplicated_scatter_view<DeviceType, Kokkos::LayoutLeft, ScatterType, NumberType>
        test_sv_left_config;
...
void test_scatter_view(int64_t n) {
  ...
  if (execution_space().concurrency() == 1) {
    test_non_duplicated_scatter_view<DeviceType, Kokkos::LayoutRight,Kokkos::Experimental::ScatterNonAtomic, ScatterType, NumberType>
        test_sv_config;
...
#endif
    test_non_duplicated_scatter_view<DeviceType, Kokkos::LayoutRight,Kokkos::Experimental::ScatterAtomic, ScatterType,NumberType>
        test_sv_config;
    test_sv_config.run_test(n);
#ifdef KOKKOS_ENABLE_SERIAL
  }
