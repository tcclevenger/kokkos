...
template <typename Rank, typename TeamHandle, typename LowerIndex,
          typename UpperIndex>
struct TeamThreadMDRange;

template <unsigned N, Iterate OuterDir, Iterate InnerDir, typename TeamHandle,
          typename LowerIndex, typename UpperIndex>
struct TeamThreadMDRange<Rank<N, OuterDir, InnerDir>, TeamHandle, LowerIndex,
                         UpperIndex> {
...
KOKKOS_DEDUCTION_GUIDE TeamThreadMDRange(TeamHandle const&,
    ...
    -> TeamThreadMDRange<Rank<N, Iterate::Default>, TeamHandle, LT, UT>;
...
template <unsigned N, Iterate OuterDir, Iterate InnerDir, typename TeamHandle,
          typename LowerIndex, typename UpperIndex>
struct ThreadVectorMDRange<Rank<N, OuterDir, InnerDir>, TeamHandle, LowerIndex,
                           UpperIndex> {
  static_assert(N >= 2u,
...
KOKKOS_DEDUCTION_GUIDE ThreadVectorMDRange(TeamHandle const&,
                                           Kokkos::Array<LT, N> const&,
                                           Kokkos::Array<UT, N> const&)
    -> ThreadVectorMDRange<Rank<N, Iterate::Default>, TeamHandle, LT, UT>;
...
template <unsigned N, Iterate OuterDir, Iterate InnerDir, typename TeamHandle,
          typename LowerIndex, typename UpperIndex>
struct TeamVectorMDRange<Rank<N, OuterDir, InnerDir>, TeamHandle, LowerIndex,
                         UpperIndex> {
...
KOKKOS_DEDUCTION_GUIDE TeamVectorMDRange(TeamHandle const&,
                                         Kokkos::Array<LT, N> const&,
                                         Kokkos::Array<UT, N> const&)
    -> TeamVectorMDRange<Rank<N, Iterate::Default>, TeamHandle, LT, UT>;
...
template <typename Rank, typename TeamHandle, typename LowerIndexType,
          typename UpperIndexType, typename Lambda>
...
template <typename Rank, typename TeamHandle, typename LowerIndexType,
          typename UpperIndexType, typename Lambda, typename ReducerValueType>
KOKKOS_INLINE_FUNCTION void parallel_reduce(
    ThreadVectorMDRange<Rank, TeamHandle, LowerIndexType, UpperIndexType> const&
        policy,
    Lambda const& lambda, ReducerValueType& val) {
  static_assert(/*!Kokkos::is_view_v<ReducerValueType> &&*/
                !std::is_array_v<ReducerValueType> &&
...
template <typename Rank, typename TeamHandle, typename LowerIndexType,
          typename UpperIndexType, typename Lambda, typename ReducerValueType>
KOKKOS_INLINE_FUNCTION void parallel_reduce(
    TeamVectorMDRange<Rank, TeamHandle, LowerIndexType, UpperIndexType> const&
        policy,
    Lambda const& lambda, ReducerValueType& val) {
  static_assert(/*!Kokkos::is_view_v<ReducerValueType> &&*/
                !std::is_array_v<ReducerValueType> &&
