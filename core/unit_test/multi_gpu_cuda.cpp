...
void operation(ResultType& result, ViewMatrixType& A, ViewVectorType& y, ViewVectorType& x) {
  ...
  Kokkos::parallel_reduce("y^TAx", policy, KOKKOS_LAMBDA(const MemberType& team, double& update) {
    const int j = team.league_rank();

    double temp=0;
    Kokkos::parallel_reduce(Kokkos::TeamVectorRange(team, N), [&] (const int i, double& innerUpdate) {
  ...
  Kokkos::initialize( argc, argv );
  {
    // EXERCISE: create a CudaStreams object
    {
      ...
      printf( "  N( %ld ) nrepeat ( %d ) problem( %g MB ) time( %g s ) bandwidth( %g GB/s )\n",
              N, nrepeat, 1.e-6*(2*N*N+4*N)*sizeof(double), time, Gbytes * nrepeat / time );
